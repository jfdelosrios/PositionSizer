...
//    if (!LabelCreate(TradingTabList, m_LblMagicNumber, first_column_start, y, first_column_start + normal_label_width, y + element_height, "m_LblMagicNumber", TRANSLATION_LABEL_MAGIC_NUMBER + ":"))                                          return false;
    ...
    if ((AdditionalTradeButtons == ADDITIONAL_TRADE_BUTTONS_LINE) || (AdditionalTradeButtons == ADDITIONAL_TRADE_BUTTONS_BOTH))
    {
        ...
        if (ShowAdditionalEntryLabel) width = int(width * 1.3);
        if (!CreateOutsideButton(m_OutsideTradeButton, 0, 0, width, element_height, "m_OutsideTradeButton", "", TRANSLATION_TOOLTIP_OUTSIDE_TRADE_BUTTON)) Print(TRANSLATION_MESSAGE_FAILED_TO_CREATE_OUTSIDE_BUTTON);
...
void CPositionSizeCalculator::MoveOutsideTradeButton()
{
    ...
    if ((sets.EntryType == Pending) || (sets.EntryType == StopLimit))
    {
        ...
        {
            // Stop
            if (sets.EntryLevel < SymbolInfoDouble(SymbolForTrading, SYMBOL_BID)) order_type = TRANSLATION_MESSAGE_SELL_STOP;
            // Limit
            else order_type = TRANSLATION_MESSAGE_SELL_LIMIT;
            // Stop Limit
            if (sets.EntryType == StopLimit) order_type = TRANSLATION_MESSAGE_SELL_STOP_LIMIT;
        }
        // Buy
        else
        {
            // Stop
            if (sets.EntryLevel > SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK)) order_type = TRANSLATION_MESSAGE_BUY_STOP;
            // Limit
            else order_type = TRANSLATION_MESSAGE_BUY_LIMIT;
            // Stop Limit
            if (sets.EntryType == StopLimit) order_type = TRANSLATION_MESSAGE_BUY_STOP_LIMIT;
        }
    }
    // Instant
    else
    {
        // Sell
        if (sets.StopLossLevel > sets.EntryLevel) order_type = TRANSLATION_MESSAGE_SELL + " " + TRANSLATION_BUTTON_ORDER_TYPE_INSTANT;
        // Buy
        else order_type = TRANSLATION_MESSAGE_BUY + " " + TRANSLATION_BUTTON_ORDER_TYPE_INSTANT;
    }
    ...
    ChartTimePriceToXY(0, 0, iTime(Symbol(), Period(), 0), sets.EntryLevel, x, y);
...
void SwitchEntryDirection()
{
    ...
    if (sets.TradeDirection == Long)
    {
        ...
        if (sets.TakeProfitLevel > 0)
        {
            ...
            {
                if (sets.TP[i] == 0) continue;
                sets.TP[i] = NormalizeDouble(sets.EntryLevel - old_tp_distance[i], _Digits);
                ObjectSetDouble(ChartID(), ObjectPrefix + "TakeProfitLine" + IntegerToString(i), OBJPROP_PRICE, sets.TP[i]);
            }
        }
...
void CPositionSizeCalculator::OnClickBtnStopPriceIncrease()
{
    ...
    // Should actually operate in TickSize rather than point as only TickSize changes make sense.
    if (TickSize > 0) sets.StopPriceLevel += TickSize * KeyBasedMultiplier();
    else sets.StopPriceLevel += _Point * KeyBasedMultiplier();
...
{
    string s = m_EdtStopPrice.Text();
    StringReplace(s, ",", "."); // Replace comma with period for normal double conversion. 
    sets.StopPriceLevel = StringToDouble(s);
    // Check and adjust for TickSize granularity.
...
//        return;
    ...
    if (sets.EntryType == Instant)
    {
        ObjectSetInteger(0, ObjectPrefix + "EntryLine", OBJPROP_SELECTABLE, false);
        if (HideEntryLineOnInstant) ObjectSetInteger(ChartID(), ObjectPrefix + "EntryLine", OBJPROP_TIMEFRAMES, OBJ_NO_PERIODS);
    }
    else
    {
